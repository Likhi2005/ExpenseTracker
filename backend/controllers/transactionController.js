...
export const getTransactions = async(req, res)=> {
    try {
        ...
        const transactions = await pool.query({
            text:`SELECT * FROM tbltransaction WHERE user_id=$1 AND createdat BETWEEN $2 AND $3 AND (description ILIKE '%' || $4 || '%' OR source ILIKE '%' || $4 || '%') ORDER BY id DESC`,
...
export const getDashboardInformation = async(req, res)=> {
    try {
        ...
        const result= await pool.query({
            ...
            FROM tbltransaction  WHERE user_id=$1 AND createdat BETWEEN $2 AND $3
            GROUP BY
        ...
        const lastTransactions = lastTransactionsResult.rows[0];
...
export const addTransaction = async(req, res)=> {
    try {
        ...
        if(!(description || source || amount)){
            return res.status(403).json({ status: "failed", message: "Please fill all fields" });
        }

        if(Number(amount)<=0){
            return res.status(403).json({ status: "failed", message: "Amount must be greater than 0" });
        }
        ...
        if(!accountInfo){
            return res.status(403).json({ status: "failed", message: "Account not found" });
        }

        if(accountInfo.account_balance<=0 || accountInfo.account_balance<Number(amount)){
            return res.status(403).json({ status: "failed", message: "Transaction failed. Insufficient balance" });
        }
        ...
        res.status(200).json({ status: "success", message: "Transaction completed successfully" });

    } catch (error){
        console.log(error);
        res.status(500).json({ status: "failed", message: "Invalid account Information" });
    }
}
...
export const transferMoneyToAccount = async(req, res)=> {
    try {
        ...
        // Check amount detail and balance for the 'from account'
